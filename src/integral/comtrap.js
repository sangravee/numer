...
class comtrap extends Component
{
  ...
  componentDidMount = async() => { 
    await api.getFunctionByName("ComTrapzoidal").then(db => {
    this.setState({
        fx:db.data.data.fx,
        a:db.data.data.a,
        b:db.data.data.b,
  ...
  onSubmit (){
    ...
    for(i=1;i<n;i++){
        sum = sum + this.func(hsum)
        console.log("h  "+hsum)
        hsum = parseFloat(hsum)+parseFloat(h);
    }
    ans = (h/2)*(this.func(parseFloat(a))+this.func(parseFloat(b))+(2*sum));
      ...
      const { size } = this.state;
        return (
          <div id="content" style={{ padding: 24, background: '#fff', minHeight: 360 }}>
            ...
            <Title style = {{textAlign: 'center'}}>Composite Trapzoidal Rule </Title>
            <br></br>
    ...
    <div>
    <h2 style = {{textAlign: 'center'}}>Table of Composite Trapzoidal</h2>
