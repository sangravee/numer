...
class oh4 extends Component
{
  ...
  componentDidMount = async() => { 
    await api.getFunctionByName("oh4_1").then(db => {
    this.setState({
      fx:db.data.data.fx,
      x:db.data.data.x,
      h:db.data.data.h,
  ...
  onSubmit (){
    ...
    data['d'][0] = fx;
    if(d==1){
      ans = (-this.func(parseFloat(x)+(parseFloat(h)*2))+(8*this.func(parseFloat(x)+parseFloat(h)))-
      (8*this.func(parseFloat(x)-parseFloat(h)))+this.func(parseFloat(x)-(parseFloat(h)*2)))/(12*parseFloat(h))
    }else if(d==2){
      ans = (-this.func(parseFloat(x)+(parseFloat(h)*2))+(16*this.func(parseFloat(x)+parseFloat(h)))-
      (30*this.func(parseFloat(x)))+(16*this.func(parseFloat(x)-parseFloat(h)))-this.func(parseFloat(x)-(parseFloat(h)*2)))
      /(12*(Math.pow(parseFloat(h),2)))
    }else if(d==3){
      ans = (-this.func(parseFloat(x)+(parseFloat(h)*3))+(8*this.func(parseFloat(x)+(parseFloat(h)*2)))-
      (13*this.func(parseFloat(x)+parseFloat(h)))+(13*this.func(parseFloat(x)-parseFloat(h)))-(8*this.func(parseFloat(x)-(parseFloat(h)*2)))
      +this.func(parseFloat(x)-(parseFloat(h)*3)))/(8*(Math.pow(parseFloat(h),3)))
    }else{
      ans = (-this.func(parseFloat(x)+(parseFloat(h)*3))+(12*this.func(parseFloat(x)+(parseFloat(h)*2)))-
      (39*this.func(parseFloat(x)+parseFloat(h)))+(56*this.func(parseFloat(x)))-
      (39*this.func(parseFloat(x)-parseFloat(h)))+(12*this.func(parseFloat(x)-(parseFloat(h)*2)))-
      this.func(parseFloat(x)-parseFloat(h)))/(6*(Math.pow(parseFloat(h),4)))
    }
      ...
      const { size } = this.state;
        return (
          <div id="content" style={{ padding: 24, background: '#fff', minHeight: 360 }}>
            ...
            <Title style = {{textAlign: 'center'}}>O(h4) </Title>
            <br></br>
    ...
    <div>
    <h2 style = {{textAlign: 'center'}}>Table of O(h4)</h2>
