...
class fwoh2 extends Component
{
  ...
  componentDidMount = async() => { 
    await api.getFunctionByName("2Forward_1").then(db => {
    this.setState({
      fx:db.data.data.fx,
      x:db.data.data.x,
      h:db.data.data.h,
  ...
  onSubmit (){
    ...
    data['d'][0] = fx;
    if(d==1){
      ans = (-this.func(parseFloat(x)+(2*parseFloat(h)))+(4*this.func(parseFloat(x)+parseFloat(h)))-(3*this.func(parseFloat(x))))/(2*parseFloat(h))
    }else if(d==2){
      ans = (-this.func(parseFloat(x)+(parseFloat(h)*3))+(4*this.func(parseFloat(x)+(2*parseFloat(h))))-
      (5*this.func(parseFloat(x)+parseFloat(h)))+(2*this.func(parseFloat(x))))/(Math.pow(parseFloat(h),2))
    }else if(d==3){
      ans = (-(3*this.func(parseFloat(x)+(4*parseFloat(h))))+(14*this.func(parseFloat(x)+(parseFloat(h)*3)))-
      (24*this.func(parseFloat(x)+(2*parseFloat(h))))+(18*this.func(parseFloat(x)+parseFloat(h)))-
      (5*this.func(parseFloat(x))))/(2*(Math.pow(parseFloat(h),3)))
    }else{
      ans = (-(2*this.func(parseFloat(x)+(5*parseFloat(h))))+(11*this.func(parseFloat(x)+(4*parseFloat(h))))-
      (24*this.func(parseFloat(x)+(3*parseFloat(h))))+(26*this.func(parseFloat(x)+(2*parseFloat(h))))-
      (14*this.func(parseFloat(x)+parseFloat(h)))+(3*this.func(parseFloat(x))))/(Math.pow(parseFloat(h),4))
    }
      ...
      const { size } = this.state;
        return (
          <div id="content" style={{ padding: 24, background: '#fff', minHeight: 360 }}>
            ...
            <Title style = {{textAlign: 'center'}}>Forward O(h2) </Title>
            <br></br>
    ...
    <div>
    <h2 style = {{textAlign: 'center'}}>Table of Forward O(h2)</h2>
