...
class forward extends Component
{
  ...
  componentDidMount = async() => { 
    await api.getFunctionByName("Forward_1").then(db => {
    this.setState({
        fx:db.data.data.fx,
        x:db.data.data.x,
        h:db.data.data.h,
  ...
  onSubmit (){
    ...
    data['d'][0] = fx;
    if(d==1){
      ans = (this.func(parseFloat(x)+parseFloat(h))-this.func(parseFloat(x)))/parseFloat(h)
    }else if(d==2){
      ans = (this.func(parseFloat(x)+(parseFloat(h)*2))-(2*this.func(parseFloat(x)+parseFloat(h)))+this.func(parseFloat(x)))/(Math.pow(parseFloat(h),2))
    }else if(d==3){
      ans = (this.func(parseFloat(x)+(parseFloat(h)*3))-(3*this.func(parseFloat(x)+(parseFloat(h)*2)))+(3*this.func(parseFloat(x)+parseFloat(h)))-this.func(parseFloat(x)))/(Math.pow(parseFloat(h),3))
    }else{
      ans = (this.func(parseFloat(x)+(parseFloat(h)*4))-(4*this.func(parseFloat(x)+(parseFloat(h)*3)))+(6*this.func(parseFloat(x)+(parseFloat(h)*2)))-(4*this.func(parseFloat(x)+parseFloat(h)))+this.func(parseFloat(x)))/(Math.pow(parseFloat(h),4))
    }
      ...
      const { size } = this.state;
        return (
          <div id="content" style={{ padding: 24, background: '#fff', minHeight: 360 }}>
            ...
            <Title style = {{textAlign: 'center'}}>Forward </Title>
            <br></br>
    ...
    <div>
    <h2 style = {{textAlign: 'center'}}>Table of Forward</h2>
